...
| `docs/`에 문서 추가/삭제 | `CLAUDE.md` "유저가 읽어야 하는 문서" 표 | 그 표가 통합본(`문서요약.html`)의 입력 목록이다. 등재하면 통합본에 자동 포함 |
| 유저 읽기용 문서 내용 변경 | (자동) `문서요약.html` | **손으로 고치지 말 것.** `npm run docs`로 재생성하며, Stop 훅이 낡았을 때 자동 실행한다. 생성기는 `execution/build-docs-summary.mjs` |
...
| 수익 모델 · 플랫폼(하이브리드/네이티브/TWA) 결정 변경 | `docs/PRD.md` §9 / §10 | §10-5 "뒤집어야 하는 조건" 해당 시 결정 전체 재검토. 결론만 승계 금지 |
| **개발 단계·기간·기술 구조 변경** | `docs/로드맵.md` | Phase별 마일스톤·미정 사항. 결정 근거는 PRD §9·§10에 있으니 중복 기재 금지, 로드맵엔 요약·참조만 |
| **로드맵 단계가 "미정 → 확정"됨** (스택·방식·범위 결정) | 해당 `docs/*-할일.md` | 확정된 단계의 세부 태스크를 즉시 할 일 문서에 추가. 로드맵/할일 분업·중복 금지는 `doc-conventions.md` §4 참조 |
