...
- **백엔드(Phase 3 확정, ☐ 미구현):** Firebase — Auth(구글+이메일), Firestore(동기화·백업), FCM/APNs(푸시), **이중 IAP**(Google Play Billing + Apple StoreKit) 결제 검증. 결정 근거 PRD §9-5, 세부 태스크 `docs/phase3-할일.md`. 설정 키는 `.env`로 관리(하드코딩 금지).
