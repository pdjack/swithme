...
| 컬럼 | 내용 |
|---|---|
| **Left** (`.col-left`) | Next Tasks 카드 (할 일 목록 `#task-list`) + Daily Reflection 카드 (목표달성률 자동계산 + 사용자 정의 항목 `#reflection-items-container`로 동적 생성, 각 /20점) |
...
- 카테고리 관리 (`#subject-manager-list`), 카테고리 추가 버튼
- 회고 항목 관리 (`#reflection-item-manager-list`), 항목 추가 버튼 (`#add-reflection-item-btn`)
...
| 패널 | 내용 |
|---|---|
| **Dashboard** (`#m-dashboard-panel`) | 2컬럼 분할 — 좌: 탭 전환(`.m-col-tabs`: 할 일/회고) → 할 일 목록 (`#m-task-list`) 또는 회고 (`#m-reflection-content`, `#m-reflection-items-container`로 동적 생성), 우: 타임테이블 탭 바(`#m-timetable-tab-bar`: 탭목록`#m-tt-tab-list` / +`#m-tt-add-btn`) + 타임테이블(`#m-timetable-root`) |
...
| **Settings** (`#m-settings-panel`) | 카테고리 관리 (`#m-subject-manager-list`) + 회고 항목 관리 (`#m-reflection-item-manager-list`, `#m-add-reflection-item-btn`) |
